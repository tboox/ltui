...
# ifdef PDC_FORCE_UTF8
    ...
    while (*src && i < n)
    {
        ...
        if (code < 0x80)
        {
            dest[i] = (unsigned char) code;
            i++;
        }
        else
            if (code < 0x800)
            {
                dest[i] = (unsigned char) ((code & 0x07c0) >> 6) | 0xc0;
                dest[i + 1] = (unsigned char) (code & 0x003f) | 0x80;
                i += 2;
            }
            else
            {
                dest[i] = (unsigned char) ((code & 0xf000) >> 12) | 0xe0;
                dest[i + 1] = (unsigned char) ((code & 0x0fc0) >> 6) | 0x80;
                dest[i + 2] = (unsigned char) (code & 0x003f) | 0x80;
                i += 3;
            }
    }
# else
